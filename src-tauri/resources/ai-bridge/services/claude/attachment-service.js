...
const TEMP_IMAGE_SUBDIR = 'jadekit-gui-images';
...
function getWorkspaceRoot() {
  return process.env.PROJECT_PATH || process.env.IDEA_PROJECT_PATH || process.cwd();
}

function getTempDir() {
  // Use workspace-based temp directory first to ensure Claude tool read permissions.
  // The tool path permission checks restrict access to paths outside the active workspace.
  const workspacePath = getWorkspaceRoot();
  if (workspacePath) {
    return path.join(workspacePath, '.jadekit', TEMP_IMAGE_SUBDIR);
  }
  // Fallback to system temp directory
  return path.join(os.tmpdir(), TEMP_IMAGE_SUBDIR);
}
  ...
  try {
    ...
    if (workspacePath) {
      ...
      if (fs.existsSync(gitExcludePath)) {
        ...
        if (!content.includes('/.jadekit/')) {
          fs.appendFileSync(gitExcludePath, `\n# JadeKit Data\n/.jadekit/\n`);
        }
      }
    }
  } catch (e) {
    // Ignore error - exclude is best-effort.
  }
}
