...
export function setupApiKey() {
  // Capture env values BEFORE clearing — clearRuntimeAuthEnv deletes them.
  ...
  // Read from captured env (set by Rust backend when starting daemon)
  debugLog('[DEBUG] Loading configuration from environment variables...');
  ...
  if (!apiKey) {
    ...
    console.error('[ERROR] Please configure ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN via Provider Management');
    throw new Error('API Key not configured');
  }

  debugLog('[DEBUG] API Key found:', apiKeySource);
  debugLog('[DEBUG] Auth type:', authType);
  if (baseUrl) {
    debugLog('[DEBUG] Base URL:', baseUrl);
  }

  // Re-set the env vars so the SDK picks them up downstream.
