...
pub(super) fn validate_server_spec(spec: &Value) -> Result<(), String> {
    if !spec.is_object() {
    ...
    if is_sse {
        let url = spec.get("url").and_then(|x| x.as_str()).unwrap_or("");
        if url.trim().is_empty() {
            return Err("sse 类型的 MCP 服务器缺少 url 字段".into());
        }
    }
    Ok(())
}
...
/// 从 MCP 条目中提取服务器规范
pub(super) fn extract_server_spec(entry: &Value) -> Result<Value, String> {
    let obj = entry
        .as_object()
