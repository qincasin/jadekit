...
/**
 * Get the dependencies base directory.
 *
 * Integration note (ccg-switch): the host app sets AI_BRIDGE_DEPS_DIR to point
 * at the directory where the Claude/Codex SDKs are installed. When unset, fall
 * back to the legacy ~/.codemoss directory so the bridge still works standalone.
 * @returns {string} The dependencies base directory path
 */
export function getCodemossDir() {
  if (process.env.AI_BRIDGE_DEPS_DIR) {
    return process.env.AI_BRIDGE_DEPS_DIR;
  }
