...
export function rewriteToolInputPaths(toolName, input) {
  ...
  const traverse = (value) => {
    ...
    if (typeof value === 'object') {
      // SECURITY FIX: rewrite both `file_path` and `path` — the danger check in
      // canUseTool reads `input.file_path || input.path`, so a tool that uses
      // `path` for a temp location must be rewritten too. Previously only
      // `file_path` was rewritten, leaving `path`-based temp writes unhandled.
      if (typeof value.file_path === 'string') {
        value.file_path = rewritePath(value.file_path);
      }
      if (typeof value.path === 'string') {
        value.path = rewritePath(value.path);
      }
 ...
 *
 * SECURITY FIX: previously this used `normalizedPath.includes(pattern)`, which
 * matched a dangerous segment ANYWHERE in the string. A legitimate project file
 * such as `/home/u/proj/src/usr/x.ts` contains `/usr/` and was wrongly denied.
 * We now resolve the path and require the dangerous pattern to be a real path
 * PREFIX (anchored at the start, or directly under the user home), so only
 * paths that actually live in a sensitive directory are blocked.
 *
...
export function isDangerousPath(filePath) {
  ...
  // Patterns are directory prefixes; a path is dangerous when it equals or sits
  // beneath one of them. Trailing separators are normalized below.
  ...
  // Resolve to an absolute path and normalize separators so prefix comparison is
  // reliable. On Windows we also lowercase for case-insensitive matching.
  let resolvedPath;
  try {
  ...
  for (const pattern of dangerousPatterns) {
    ...
    // A pattern ending in a separator is a directory prefix: the path is
    // dangerous if it is the directory itself or anything beneath it.
    const isDirPattern = normalizedPattern.endsWith('/') || normalizedPattern.endsWith('\\');
    if (isDirPattern) {
      const dirNoSlash = normalizedPattern.slice(0, -1);
      if (normalizedPath === dirNoSlash || normalizedPath.startsWith(normalizedPattern)) {
        return true;
      }
    } else {
      // Exact-file pattern (e.g. .claude/.credentials.json): match the full path
      // or a path that ends with that file under any directory boundary.
      if (
        normalizedPath === normalizedPattern ||
        normalizedPath.endsWith((isWindows ? '\\' : '/') + basename(normalizedPattern)) &&
